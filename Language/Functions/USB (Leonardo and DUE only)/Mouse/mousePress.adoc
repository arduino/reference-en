...
[float]
=== Description
Sends a button press to a connected computer. A press is the equivalent of clicking and continuously holding the mouse button. A press is cancelled with link:mouseRelease{ext-relative}[Mouse.release()].

Before using `Mouse.press()`, you need to start communication with link:mouseBegin{ext-relative}[Mouse.begin()].
