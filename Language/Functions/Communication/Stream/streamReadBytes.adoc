...
[float]
=== Description
`readBytes()` read characters from a stream into a buffer. The function terminates if the determined length has been read, or it times out (see link:streamSetTimeout{ext-relative}[setTimeout()]).
