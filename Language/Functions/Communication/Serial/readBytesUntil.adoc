...
[float]
=== Description
Serial.readBytesUntil() reads characters from the serial buffer into an array. The function terminates if the determined length has been read, if it times out (see link:../settimeout[Serial.setTimeout()]), or if the terminator character is detected. The function returns the characters up to the last character before the supplied terminator. The terminator itself is not returned in the buffer.
